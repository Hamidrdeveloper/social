...
const ButtonsProfileScreen = ({navigation, userId, post, follow}) => {
  const [changeFollowText, setChangeFollowText] = useState(follow);
  ...
  const following = post?.item?.user.you_follow.check;
  useEffect(() => {
    setChangeFollow(follow);
    setChangeFollowText(follow);
  }, [follow])
...
export default ButtonsProfileScreen;
